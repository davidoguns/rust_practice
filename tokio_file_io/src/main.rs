        ...
        Ok(_bytes_read) => {
            if let Ok(delete_index) = String::from(input.trim()).parse::<usize>() {
                if (0..book_db.len()).contains(&delete_index) {
                    ...
                    println!(
                        "Book index to delete is out of range! Pick between 0 and {}",
                        book_db.len() - 1
                    );
                }
            } else {
        ...
        Ok(_bytes_read) => {
            if let Ok(view_index) = String::from(input.trim()).parse::<usize>() {
                if (0..book_db.len()).contains(&view_index) {
                    ...
                    println!(
                        "Book index to view is out of range! Pick between 0 and {}",
                        book_db.len() - 1
                    );
                }
            } else {
...
fn _load_books(book_db: &mut LinkedList<Book>) {
    ...
    if !book_db.is_empty() {
        ...
        overwrite = match std::io::stdin().read_line(&mut input) {
            Ok(_bytes_read) => match input.trim().to_lowercase().as_str() {
                "a" => false,
                "o" => true,
                _ => return,
            },
            ...
            match File::open(&Path::new(input.trim())) {
                Ok(mut file) => {
                    ...
                    match file.read_to_string(&mut input) {
                                ...
                                if let Ok(num_books) = num_books_line.parse::<usize>() {
                                    for index in 0..num_books {
                                        if let Some(book_line) = line_itr.next() {
                                            ...
                                            println!(
                                                "Unexpected end of books db at line {}",
                                                index + 1
                                            );
                                            return;
                                        }
                                    ...
                                    println!(
                                        "Couldn't parse number of books from the first line [{}]",
                                        num_books_line
                                    );
                                    return;
                                }
                        ...
                        Err(_e) => {
                            println!(
                                "Error reading contents of file {} into string",
                                input.trim()
                            );
                        }
                    }
...
fn main() {
    ...
    loop {
        ...
        println!("=========================================================================");
        println!(
            "Welcome to the books database. There are {} books in the database.",
            book_db.len()
        );
        println!("1. Add book");
